    ...
    <Card className="flex flex-col overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1 duration-300">
      <CardHeader className="flex-row gap-4 items-center p-4">
        <div className="relative shrink-0">
          <Image
            src={mechanic.avatar.imageUrl}
            alt={language === 'ar' ? mechanic.name : mechanic.name_en}
            width={64}
            height={64}
            className="rounded-full border-2 border-primary/50"
            data-ai-hint={mechanic.avatar.imageHint}
          />
          {mechanic.available && (
        ...
        <div className="flex-1">
          <CardTitle className="text-base font-bold">{language === 'ar' ? mechanic.name : mechanic.name_en}</CardTitle>
      ...
      <CardContent className="p-4 pt-0">
         <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{language === 'ar' ? `يبعد ${mechanic.distance} كم` : `${mechanic.distance} km away`}</span>
          </div>
      </CardContent>
